...
type
  ...
  public
    { Public declarations }
    procedure WndProc(var I_Msg: TMessage); override;
...
begin
  ...
  S := TmpCharB;
  if not (TmpCharB in AllBrackets) then begin
...
//      and (not (tmpstr[i] in [';', '(', ')', ',', Chr(VK_SPACE), Chr(VK_TAB), #10, Chr(VK_RETURN)])) then
    and (UpCase(tmpstr[i]) in ['0'..'9', 'A'..'Z', '_', '$', '.', #127..#255]) then
...
//    end;
    ...
    while (_wordBeg > 0) do
    begin
      if not ((UpCase(_tmpstr[_wordBeg]) in ['0'..'9', 'A'..'Z', '_', '$', #127..#255])) then
      begin
        _wordBeg := _wordBeg + 1;
        break;
      end else begin
        if UpCase(_tmpstr[_wordBeg]) = #161 then begin //全角中文空格
          _wordBeg := _wordBeg + 1;
          break;
        end;
        if _wordBeg = 1 then
...
begin
  ...
  if (I_Direction in [edLeft, edAll]) then begin
    ...
    begin
      ...
      if ((UpCase(tmpstr[i]) in ['0'..'9', 'A'..'Z', '_', #127..#255])
        or (_pos > 0))
        then
      begin //当前光标前为组成单词的字符才向前搜索
        Continue;
...
procedure THoSynEdit.WndProc(var I_Msg: TMessage);
var
  _i: integer;
  _result: Integer;
//  s: string;
...
//      else
      ...
      if I_Msg.Result = 0 then
        if _result = 1 then
          I_Msg.Result := _result;
